...
class ClassifierRuntime:
    def __init__(self, config: dict[str, Any], base_dir: Path) -> None:
        ...
        self.last_logged_stable_state = "unknown"
        self.last_logged_quick_state = "unknown"
    ...
    def predict(self, image: np.ndarray) -> dict[str, Any]:
        ...
        if self.last_logged_stable_state != self.stable_state:
            print(
                f"[VISION_STABLE] {self.last_logged_stable_state}->{self.stable_state} raw={clutter_raw:.3f} smooth={clutter_smooth:.3f}",
                flush=True,
            )
            self.last_logged_stable_state = self.stable_state

        if self.last_logged_quick_state != self.quick_state:
            print(
                f"[VISION_QUICK] {self.last_logged_quick_state}->{self.quick_state} raw={clutter_raw:.3f} smooth={clutter_smooth:.3f}",
                flush=True,
            )
            self.last_logged_quick_state = self.quick_state
...
class SerialSender:
    ...
    def maybe_send(self, snapshot: dict[str, Any]) -> None:
        ...
        if self.last_send_ts == 0.0:
            ...
            burst_count = self.change_burst_count
        elif self.send_on_change:
            burst_count = 1 if heartbeat_ready else 0
        else:
            burst_count = 1 if interval_ready else 0
        ...
        try:
            ...
            while True:
                ...
                if roi_frame.size > 0 and (now - last_infer_ts) >= infer_interval_s:
                    ...
                    if infer_ms > infer_max_ms:
                        infer_max_ms = infer_ms
                    if (now - last_perf_log_ts) >= PERF_LOG_INTERVAL_S:
                        avg_ms = infer_total_ms / infer_count if infer_count > 0 else 0.0
                        fps = 1000.0 / avg_ms if avg_ms > 0.0 else 0.0
                        print(f"[PERF] infer_avg_ms={avg_ms:.2f} infer_max_ms={infer_max_ms:.2f} fps_est={fps:.1f}", flush=True)
...
# @APP.route("/")
# def index():
#     return "网页预览与状态轮询已关闭，当前仅保留串口主链路和 /api/status 接口。"


# @APP.route("/video_feed")
# def video_feed():
#     return ("video preview disabled", 404)
