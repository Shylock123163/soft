...
1. ✅ `web-ui/app` 主工程已建立（React 18 + Vite 5 + TypeScript）
2. ✅ 控制台壳子已完成（5 页面路由 + 3D 场景 + 对话 + 监控）
3. 预留 `Assets` 页面（待实现）
4. 补 `image-to-3d` 后端接口（待实现）
5. 把 `glb` 加载到 `react-three-fiber` 场景里（待实现）

## 9. 一句话方案

**Replicate MCP 用来辅助开发和调试，网站正式运行时走你自己的后端资产接口；图片转 3D 属于”资产生成链路”，应该挂到控制台里，但不应侵入机器人实时控制链路，3D 预览使用 `@react-three/fiber` + `@react-three/drei`。**
